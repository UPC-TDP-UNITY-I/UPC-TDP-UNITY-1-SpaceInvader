...
public class FSMEngine : MonoBehaviour {
	...
	private void ChangeState(Dictionary<string, object> dict) {
		...
		case "move":
			...
			break;
        case "shoot":
            float bulletSpeed = (float)dict["bulletSpeed"];
            float frecuency = (float)dict["frecuency"];

            newState = new FSMC_Shoot(this.gameObject);
            ((FSMC_Shoot)newState).Shoot(bulletSpeed, frecuency);
            break;
		}
	...
	void Update () {
		if (this.Behaviours.Count > 0) {
			if (mPersistenIndex != mIndex) {
				...
				Dictionary<string, object> dict = Utils.ParseScript (rawScript);
